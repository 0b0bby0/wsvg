...
#LICENSE:
#
#convert is part of wsvg, which is free software; you can redistribute it and/or modify
...
def rgb_to_hsl(in_col):
	...
	Input should be a tuple of integers (R, G, B) where each is between 0 and 255.
	Output is a tuple of floats between 0.0 and 1.0..
	'''
...
class Color:
	...
	def __len__(self):
		"""Override default length behavior"""
		return len(self.col)


	def __getitem__(self, index):
		"""Override default getitem behavior"""
		if self.get_format() == 'hex':
			return str(rgb_to_hex(self.col))[index]
		else:
			return str(self.col[index])


	def __iter__(self):
		for item in iter(self.col):
			yield item


	def lower(self):
		if self.get_format() == 'hex':
			return str(rgb_to_hex(self.col)).lower()
		else:
			return self.col
	...
	def square(self):
		...
		#add all the colors together
		colors = [self.col, tuple(analagous[0]), tuple(analagous[1]), tuple(analagous[2])]
...
def test():
	...
	assert x.split_complementary() == ['#01f490', '#f41601', '#f401de'] #these seem to be off by one
	...
	assert x.tints() == ['#01f490', '#11fe9d', '#2cfea8', '#46feb3', '#61febd', '#7bfec8', '#95ffd3', '#b0ffde', '#caffe9', '#e5fff4', '#ffffff']
	assert x.shades() == ['#01f490', '#01dc82', '#01c373', '#01ab65', '#019256', '#017a48', '#00623a', '#00492b', '#00311d', '#00180e', '#000000']
	assert x.saturate() == ['#01f490', '#01f490', '#01f490', '#01f490', '#01f490', '#00f490', '#00f590', '#00f590', '#00f590', '#00f590', '#00f590']
	assert x.desaturate() == ['#01f490', '#0de88e', '#19dc8c', '#25d08a', '#32c387', '#3eb785', '#4aab83', '#569f81', '#62937f', '#6e877d', '#7a7a7a']
	...
	assert x.split_complementary() == [(1,244,144), (244,22,1), (244,1,222)] #these seem to be off by one
	...
	assert x.tints() == [(1,244,144), (17,254,157), (44,254,168), (70,254,179), (97,254,189), (123,254,200), (149,255,211), (176,255,222), (202,255,233), (229,255,244), (255,255,255)]
	assert x.shades() == [(1,244,144), (1,220,130), (1,195,115), (1,171,101), (1,146,86), (1,122,72), (0,98,58), (0,73,43), (0,49,29), (0,24,14), (0,0,0)]
	assert x.saturate() == [(1,244,144), (1,244,144), (1,244,144), (1,244,144), (1,244,144), (0,244,144), (0,245,144), (0,245,144), (0,245,144), (0,245,144), (0,245,144)]
	assert x.desaturate() == [(1,244,144), (13,232,142), (25,220,140), (37,208,138), (50,195,135), (62,183,133), (74,171,131), (86,159,129), (98,147,127), (110,135,125), (122,122,122)]
